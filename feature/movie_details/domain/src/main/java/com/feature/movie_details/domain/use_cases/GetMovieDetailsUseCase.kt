...
class GetMovieDetailsUseCase @Inject constructor(private val movieDetailRepo: MovieDetailRepo) {

    operator fun invoke(id: String, apiKey: String): Flow<UiEvent<MovieDetails>> = flow {
        emit(UiEvent.Loading())
        emit(UiEvent.Success(movieDetailRepo.getMovieDetails(id, apiKey)))
    }.catch {
